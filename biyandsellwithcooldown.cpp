...
int stockprofit(int *arr, int n, bool buy, int index)
{
    ...
    {
        profit = max(arr[n] + stockprofit(arr, n + 2, true, index), stockprofit(arr, n + 1, false, index));
    }
    return profit;
}
int memo(int *arr, int n, int buy, int index, vector<vector<int>> dp)
{
    ...
    {
        profit = max(arr[n] + memo(arr, n + 2, true, index, dp), memo(arr, n + 1, false, index, dp));
    }
